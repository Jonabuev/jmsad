import React, { useEffect, useState } from "react";
import {
  GoogleOAuthProvider,
  useGoogleLogin,
} from "@react-oauth/google";
import { googleAuth } from "@/api/authApi";
import { saveTokens } from "@/utils/tokenUtils";
import { setCookie } from "@/utils/cookieUtils";
import { useRouter } from "next/router";
import { useTranslation } from "next-i18next";
import { useDispatch } from "react-redux";
import { AppDispatch } from "@/component/store/store";
import { fetchUserProfile } from "@/component/store/auth/authSlice";
import styles from "./GoogleLoginButton.module.scss";

interface GoogleLoginButtonProps {
  onSuccess?: () => void;
  onError?: (error: any) => void;
  className?: string;
}

// Внутренний компонент с кастомной кнопкой (должен быть внутри GoogleOAuthProvider)
const CustomGoogleButton: React.FC<{
  onSuccess?: () => void;
  onError?: (error: any) => void;
  className?: string;
  currentLocale: string;
}> = ({ onSuccess, onError, className, currentLocale }) => {
  const { t } = useTranslation("common");
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const [isLoading, setIsLoading] = useState(false);

  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setIsLoading(true);
      try {
        // Отправляем access_token на наш backend
        const { data } = await googleAuth(tokenResponse.access_token);
        saveTokens(data.access, data.refresh);
        
        setCookie("user", JSON.stringify(data.user), {
          expires: 7,
          path: '/',
          secure: false,
          sameSite: 'lax'
        });

        await dispatch(fetchUserProfile());
        
        if (onSuccess) {
          onSuccess();
        } else {
          await router.push("/profile");
        }
      } catch (err) {
        console.error("Google Login Error:", err);
        if (onError) {
          onError(err);
        }
      } finally {
        setIsLoading(false);
      }
    },
    onError: (error) => {
      console.error("Google Login Failed:", error);
      if (onError) {
        onError(new Error("Google login failed"));
      }
    },
    flow: 'implicit',
  });

  return (
    <button
      onClick={() => login()}
      disabled={isLoading}
      className={`${styles.googleButton} ${isLoading ? styles.loading : ''} ${className}`}
    >
      <svg className={styles.googleIcon} viewBox="0 0 24 24">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
      </svg>
      <span className={styles.googleButtonText}>
        {isLoading ? t("common.loading") : t("registration.sign_in_with_google")}
      </span>
    </button>
  );
};

const GoogleLoginButton: React.FC<GoogleLoginButtonProps> = ({
  onSuccess,
  onError,
  className = ""
}) => {
  const router = useRouter();
  const { i18n } = useTranslation("common");
  
  const initialClientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || "";
  const [clientId, setClientId] = useState<string>(initialClientId);
  const [currentLocale, setCurrentLocale] = useState<string>(() => {
    return router.locale || i18n.language || 'ru';
  });
  
  // Отслеживаем изменение локали
  useEffect(() => {
    const newLocale = router.locale || i18n.language || 'ru';
    if (newLocale !== currentLocale) {
      setCurrentLocale(newLocale);
    }
  }, [router.locale, i18n.language, currentLocale]);
  
  useEffect(() => {
    if (!clientId && typeof process !== 'undefined') {
      const fromEnv = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || "";
      if (fromEnv) setClientId(fromEnv);
    }
  }, [clientId]);
  
  // Проверяем, что мы на клиенте
  if (typeof window === 'undefined') {
    return null;
  }

  if (!clientId) {
    return (
      <button 
        className={`${styles.googleButton} ${styles.error} ${className}`}
        disabled
      >
        <svg className={styles.googleIcon} viewBox="0 0 24 24">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
        <span className={styles.googleButtonText}>Ошибка конфигурации</span>
      </button>
    );
  }

  return (
    <GoogleOAuthProvider clientId={clientId} key={`provider-${currentLocale}`}>
      <CustomGoogleButton
        onSuccess={onSuccess}
        onError={onError}
        className={className}
        currentLocale={currentLocale}
      />
    </GoogleOAuthProvider>
  );
};

export default GoogleLoginButton;
